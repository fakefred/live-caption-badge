...
void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param) {
	...
	case ESP_GAP_BLE_SCAN_RESULT_EVT: {
		...
		case ESP_GAP_SEARCH_INQ_RES_EVT:
			...
			if (adv_name != NULL) {
				if (adv_name_len > 0 &&
				    strstr((char *)adv_name, DEVICE_NAME_FILTER) != NULL) {
					ESP_LOGI(GATTC_TAG, "searched device %s", (char *)adv_name);
					if (device_count < MAX_DEVICES) {
						memcpy(&deviceList[device_count].bda,
						       &scan_result->scan_rst.bda,
						       sizeof(esp_bd_addr_t));
						memcpy(&deviceList[device_count].addr_type,
						       &scan_result->scan_rst.ble_addr_type,
						       sizeof(esp_ble_addr_type_t));
						device_count++;
					}
					// if (connect == false) {
					//     connect = true;
			...
			if (device_count > 0) {
				for (index = 0; index < device_count; index++) {
					esp_ble_gattc_open(
					    ...
					    deviceList[index].bda, deviceList[index].addr_type,
					    true);
				}
			}
			break;
