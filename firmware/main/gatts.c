...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] = {
    ...
    [IDX_CHAR_VAL_A] = {{ESP_GATT_AUTO_RSP},
                        {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_TEST_A,
                         ...
                         sizeof(ipAddrComplete), (uint8_t *)ipAddrComplete}},
...
void example_prepare_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env,
                                     esp_ble_gatts_cb_param_t *param) {
	ESP_LOGI(GATTS_TABLE_TAG, "prepare write, handle = %d, value len = %d", param->write.handle,
	         param->write.len);
...
void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if,
	...
	case ESP_GATTS_REG_EVT: {
		...
		if (raw_adv_ret) {
			ESP_LOGE(GATTS_TABLE_TAG, "config raw adv data failed, error code = %x ",
			         raw_adv_ret);
		}
		adv_config_done |= ADV_CONFIG_FLAG;
		...
		if (raw_scan_ret) {
			ESP_LOGE(GATTS_TABLE_TAG,
			         "config raw scan rsp data failed, error code = %x", raw_scan_ret);
		}
		adv_config_done |= SCAN_RSP_CONFIG_FLAG;
#else
		// config adv data
	...
	case ESP_GATTS_CONNECT_EVT:
		...
		/* For the iOS system, please refer to Apple official documents about the BLE
