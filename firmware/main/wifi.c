...
static void event_handler(void *arg, esp_event_base_t event_base, int32_t event_id,
	...
	if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
		...
		for (int i = 0; i < 4; i++) {
			...
			memcpy(ipAddrComplete + offset, ipaddrstr[i],
			       len);   // Copy the current string
...
void wifi_init(void) {
	    ...
	    .sta =
	        {
	             ...
	             * standards (password len => 8). If you want to connect the device to
	...
	if (bits & WIFI_CONNECTED_BIT) {
		...
		ESP_LOGI(TAG, "ip variable (int): %d.%d.%d.%d", ipaddr[0], ipaddr[1], ipaddr[2],
		         ipaddr[3]);
		ESP_LOGI(TAG, "ip variable (string): %s.%s.%s.%s", ipaddrstr[0], ipaddrstr[1],
		         ipaddrstr[2], ipaddrstr[3]);
