    ...
    case ESP_GATTC_READ_CHAR_EVT:
        if(p_data->read.status != ESP_GATT_OK){
            ...
            if(!readIP){
                ...
                memcpy(&userList[device_count].name, &test, strlen(test)*sizeof(char));
                ESP_LOGI(GATTC_TAG, "name(userList): %s", userList[device_count].name);
                readIP = !readIP;
            }else{
                memcpy(&userList[device_count].ip, p_data->read.value, p_data->read.value_len);
