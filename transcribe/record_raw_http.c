...
static const char remote_device_name[] = "BLE Device";
...
static esp_bt_uuid_t remote_filter_char_uuid = {
    .len = ESP_UUID_LEN_16,
    .uuid = {.uuid16 = REMOTE_READ_CHAR_UUID,},
};
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_READ_CHAR_EVT:
        if(p_data->read.status != ESP_GATT_OK){
            ...
            memcpy(targetIP, p_data->read.value, p_data->read.value_len);
            ESP_LOGI(GATTC_TAG, "targetIP: %s", targetIP);
        }
        break;
...
#define SAMPLE_DEVICE_NAME          "Live Caption Badge 1"
...
char* currentText;
uint16_t ipaddr[4] = {0, 0, 0, 0};
char ipaddrstr [4][5] = {0};
...
static uint8_t raw_adv_data[] = {
        ...
        0x03, 0x03, 0xFF, 0x00,
        /* device name */
        0x0D, 0x09, 'L', 'I', 'V','E', '_', 'C', 'A', 'P', 'T', 'I', 'O', 'N'
};
...
//static const uint8_t char_value[4]                 = {0x11, 0x22, 0x33, 0x77};
//static uint8_t char_value[13] = "192.168.0.120";
...
static const esp_gatts_attr_db_t gatt_db[HRS_IDX_NB] =
{
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&GATTS_CHAR_UUID_TEST_A, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      GATTS_DEMO_CHAR_VAL_LEN_MAX, sizeof(ipAddrComplete), (uint8_t *)ipAddrComplete}},
...
*/
#define EXAMPLE_ESP_WIFI_SSID      "TP-Link_F9C9"
#define EXAMPLE_ESP_WIFI_PASS      "68299859"
...
static void event_handler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        //memcpy(&temp, (u32_t)(event->ip_info.ip), 4);
        ipaddr[0] = esp_ip4_addr1_16(&event->ip_info.ip);
        ipaddr[1] = esp_ip4_addr2_16(&event->ip_info.ip);
        ipaddr[2] = esp_ip4_addr3_16(&event->ip_info.ip);
        ipaddr[3] = esp_ip4_addr4_16(&event->ip_info.ip);
        for(int i = 0; i < 4; i++){
            intToStr(ipaddr[i], ipaddrstr[i]);
        }
        size_t offset = 0;
        for (int i = 0; i < 4; i++) {
            size_t len = strlen(ipaddrstr[i]);  // Get the length of the current string
            memcpy(ipAddrComplete + offset, ipaddrstr[i], len);  // Copy the current string
            offset += len;  // Update offset after copying the string
            if (i < 3) {  // Add the dot only between the IP parts, not at the end
                memcpy(ipAddrComplete + offset, &dot, 1);  // Copy the dot
                offset++;  // Increment offset for the dot
            }
        }
        s_retry_num = 0;
...
void wifi_init_sta(void)
{
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ...
        ESP_LOGI(TAG, "ip variable (int): %d.%d.%d.%d", ipaddr[0], ipaddr[1], ipaddr[2], ipaddr[3]);
        ESP_LOGI(TAG, "ip variable (string): %s.%s.%s.%s", ipaddrstr[0], ipaddrstr[1], ipaddrstr[2], ipaddrstr[3]);
        ESP_LOGI(TAG, "ip variable (single string): %s", ipAddrComplete);
    ...
    if (evt->type == INPUT_KEY_SERVICE_ACTION_CLICK) {
        switch ((int)evt->data) {
            ...
            case INPUT_KEY_USER_ID_REC:
                ...
                audio_element_set_uri(http_stream_writer2, "http://192.168.0.181:7000/upload");
